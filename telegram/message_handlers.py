...
def handle_news_request(user, message_id, page = None):
    ...
    if page:
        ...
        request_url = f'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getNews.php?count=5&page={page}'
    else:
        logging.info(f'[NEWS REQUEST] REQUESTING FIRST PAGE')
        request_url = 'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getNews.php?count=5&page=1'
    ...
    try:
        ...
        response_message = '\n\n'.join(
                [f'{num+1}. <a href="https://dev.xn--37-9kcqjffxnf3b.xn--p1ai{entry.get("DETAIL_PAGE_URL")}">{entry.get("NAME")}</a>' 
...
def handle_events_request(user, message_id, page = None):
    ...
    if page:
        ...
        request_url = f'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getEvents.php?count=5&page={page}'
    else:
        logging.info(f'[EVENTS REQUEST] REQUESTING FIRST PAGE')
        request_url = 'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getEvents.php?count=5&page=1'
    request = requests.get(request_url)
    try:
        response = request.json()
        result = response.get('result')
...
def handle_services_request(user, message_id, service = None):
    ...
    if service:
        ...
        request_url = f'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getServices.php?section={service}'
    else:
        logging.info(f'[SERVICE REQUEST] REQUESTING SERVICES LIST')
        request_url = 'https://dev.xn--37-9kcqjffxnf3b.xn--p1ai/api/bot/getServices.php'
    request = requests.get(request_url)
    try:
        print(request.text)
        response = request.json()
